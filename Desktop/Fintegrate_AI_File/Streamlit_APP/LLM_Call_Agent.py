...
class LLMCallAgent:
    ...
    def __init__(self, 
        ...
        Args:
            openai_api_key (str): OpenAI API key (if None, will try to import from Stock_Trend_Storage_Agent)
            deepseek_api_key (str): DeepSeek API key (if None, will try to import from Stock_Trend_Storage_Agent)
            default_provider (str): Default LLM provider ("openai" or "deepseek")
            default_model (str): Default model to use
        """
        # Try to import API keys from Stock_Trend_Storage_Agent if not provided
        if not openai_api_key:
            try:
                from Stock_Trend_Storage_Agent import OPEN_API_KEY
                openai_api_key = OPEN_API_KEY
                logging.info("📥 Using OpenAI API key from Stock_Trend_Storage_Agent")
            except ImportError:
        ...
        if not deepseek_api_key:
            try:
                from Stock_Trend_Storage_Agent import DEEPSEEK_API
                deepseek_api_key = DEEPSEEK_API
                logging.info("📥 Using DeepSeek API key from Stock_Trend_Storage_Agent")
            except ImportError:
                deepseek_api_key = os.getenv('DEEPSEEK_API_KEY')
                if deepseek_api_key:
                    logging.info("📥 Using DeepSeek API key from environment variable")
                else:
    ...
    def get_provider_status(self) -> Dict:
        ...
        return {
            "openai": "enabled" if self.openai_client else "disabled",
            "deepseek": "enabled" if self.deepseek_client else "disabled",
...
def main():
    ...
    if not openai_key and not deepseek_key:
        print("⚠️ No API keys found. Set OPENAI_API_KEY or DEEPSEEK_API_KEY environment variables.")
        return
