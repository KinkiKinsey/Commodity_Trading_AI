...
def openai_api_call(prompt, model="gpt-4o", max_tokens=10000):
    ...
    try:
        ...
        if "insufficient_quota" in error_msg or "429" in error_msg:
            ...
            try:
                return deepseek_api_call(prompt, model="deepseek-chat")
...
def process_single_llm_batch(args):
    ...
    try:
        response_text = deepseek_api_call(batch_prompt, model="deepseek-chat")
...
def summarize_news_trends_with_llm(trend_json: dict, ticker: str, use_multiprocessing: bool = True) -> dict:
    ...
    if not use_multiprocessing:
        ...
        # Use multiprocessing to process LLM batches in parallel
        with Pool(processes=min(len(process_args), os.cpu_count())) as pool:
            results = pool.map(process_single_llm_batch, process_args)
...
def analyze_stock_trends(ticker: str, force_update: bool = False, use_multiprocessing: bool = True):
    """
    ...
    try:
        # Step 1: Get price trend segments
        print("1️⃣ Analyzing price trends...")
        time_list = plot_clean_zigzag_segments(ticker, start_date, end_date, order=7)
        ...
        summary_json = summarize_news_trends_with_llm(news_result, ticker=ticker, use_multiprocessing=use_multiprocessing)
        ...
        # Step 7: Create metadata (no local file saving)
...
if __name__ == "__main__":
    ...
    try:
        historical_json, current_json, metadata = analyze_stock_trends(ticker, force_update, use_multiprocessing)
