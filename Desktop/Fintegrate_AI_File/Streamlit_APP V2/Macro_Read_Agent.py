...
class MacroReadAgent:
    ...
    def __init__(self):
        ...
        # Initialize LLM client (will auto-import API keys from centralized keys)
        ...
        print(f"🔑 DeepSeek API Key: {'✅ Available' if self.llm_client.deepseek_api_key else '❌ Missing'}")
        print(f"🔑 OpenAI API Key: {'✅ Available' if self.llm_client.openai_api_key else '❌ Missing'}")
    
    def check_data_freshness(self) -> Dict[str, Any]:
        """
        Check if macro data is fresh by reading meta_data from Macro_Analyst file
        ...
        try:
            ...
            try:
                ...
                # Redis configuration (same as Macro DB Agent)
                redis_config = {
    ...
    def trigger_macro_update(self) -> bool:
        """
        Trigger Macro DB Agent update by running it as a subprocess
        ...
        try:
            ...
            print("📡 Running: python3 Macro_DB_Agent.py")
            
            # Run Macro DB Agent
            result = subprocess.run(
                ['python3', 'Macro_DB_Agent.py'],
                capture_output=True,
                text=True,
                cwd=os.getcwd()
            )
            ...
            if result.returncode == 0:
                print("✅ Macro DB Agent completed successfully!")
                print("📊 Macro data updated and ready for queries!")
                return True
            else:
                print(f"❌ Macro DB Agent failed with return code: {result.returncode}")
    ...
    def read_macro_data(self) -> Dict[str, Any]:
        """
        Read macro data from Redis database
    ...
    def process_user_query(self, user_query: str) -> str:
        """
        Process user query and provide relevant macro data information
        ...
        try:
            ...
            if freshness_status['needs_update']:
                ...
                # Trigger update
                update_success = self.trigger_macro_update()
            ...
            response = self._generate_llm_response(user_query, analysis, metadata, indicators)
    ...
    def _generate_llm_response(self, query: str, analysis: str, metadata: Dict, indicators: List[str]) -> str:
        """
        Generate intelligent LLM-powered response based on user query and macro data
        ...
        try:
            ...
            llm_response = self.llm_client.call_deepseek(prompt)
