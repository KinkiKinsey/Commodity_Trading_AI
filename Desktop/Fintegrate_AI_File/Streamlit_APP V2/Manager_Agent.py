...
class ManagerAgentResultPydantic(BaseModel):
    ...
    query_for_market_expectation: str = Field(description="Query for market expectation agent")
    query_for_revenue_segmentation: str = Field(description="Query for revenue segmentation agent")
    query_for_macro_analyst: str = Field(description="Query for macro analyst agent")
...
class ManagerAgent:
    ...
    def __init__(self, redis_config: Optional[Dict] = None, user_id: Optional[str] = None):
        ...
        # Initialize LLM Call Agent (will auto-import API keys from centralized keys)
        ...
        print(f"   - DeepSeek: {'✅ Available' if self.manager_agent.deepseek_api_key else '❌ Missing'}")
        print(f"   - OpenAI: {'✅ Available' if self.manager_agent.openai_api_key else '❌ Missing'}")
    ...
    def _setup_structured_llm(self):
        """Setup centralized structured LLM for intelligent routing"""
        try:
            self.structured_llm = self.manager_agent.get_structured_llm(ManagerAgentResultPydantic)
        except Exception as e:
    ...
    def process_manager_query(self, user_query: str, ticker: str) -> ManagerAgentResult:
        ...
        1. MARKET EXPECTATION AGENT:
           - Database: This agent contains ALL stock trend information during different historical fundamental, macro, and all other events for any specific stock trend
           - Best for: Comprehensive stock trend analysis, historical pattern matching, price behavior during various events
           - Example queries: "Find historical trends for this stock during similar events", "Analyze stock performance patterns"
           - Decision: Call this agent if necessary for ANY cause related to stock trends, historical analysis, or price behavior. This agent should be called frequently as it contains comprehensive trend data.
        
        2. REVENUE SEGMENTATION AGENT:
           - Database: Revenue breakdown, customer segments, business fundamentals
           - Best for: Revenue impact analysis of fundamental changes, business model shifts, product/service changes
           - Example queries: "How will this affect the company's revenue?", "Analyze business model impact"
           - Decision: Call this agent if ANY information about fundamentals, business operations, products/services, revenue drivers, cost structures, or legal issues that might impact product/service revenue
        
        3. MACRO ANALYST AGENT:
           - Database: Macroeconomic indicators, policy changes, economic environment data
           - Best for: Sector-level macro analysis, economic policy impacts on specific sectors
           - Example queries: "How do macro events affect this sector?", "Analyze sector-level economic impacts"
           - Decision: Call this analyst if ANY news about macro side to retain the ticker's sector impaction during macro events
        
        IMPORTANT ROUTING GUIDELINES:
        - Market Expectation Agent: Call this agent FREQUENTLY as it contains comprehensive stock trend data for any analysis
        - Revenue Segmentation Agent: Call for ANY fundamental, business, product, revenue, cost, or legal impacts
        - Macro Analyst Agent: Call for ANY macro events that could affect the ticker's sector
        ...
        5. Remember: Market Expectation Agent should be called frequently for comprehensive trend analysis
        
        OUTPUT FORMAT:
        - Decision_call_market_expectation: 1 if query involves stock trends, historical analysis, or price behavior (call frequently), 0 otherwise
        - Decision_call_revenue_segmentation: 1 if query involves fundamentals, business, products, revenue, cost, or legal impacts, 0 otherwise  
        - Decision_call_macro_analyst: 1 if query involves macro events affecting the ticker's sector, 0 otherwise
        ...
        try:
            ...
            return ManagerAgentResult(
                ...
                query_for_market_expectation=result.query_for_market_expectation,
                query_for_revenue_segmentation=result.query_for_revenue_segmentation,
                query_for_macro_analyst=result.query_for_macro_analyst
            )
        except Exception as e:
    ...
    def _fallback_routing(self, user_query: str, ticker: str) -> ManagerAgentResult:
        ...
        # Enhanced keyword-based routing with updated logic
        market_keywords = ['trend', 'pattern', 'historical', 'price', 'behavior', 'earnings', 'stock performance', 'stock', 'market', 'performance', 'analysis', 'lawsuit', 'legal', 'antitrust', 'regulatory']
        revenue_keywords = ['revenue', 'partnership', 'business model', 'customer', 'service', 'profitability', 'cost', 'spending', 'fundamental', 'product', 'legal', 'business', 'lawsuit', 'antitrust', 'regulatory', 'competition', 'market access']
        macro_keywords = ['interest rate', 'federal reserve', 'economic', 'policy', 'macro', 'inflation', 'fed', 'powell', 'sector', 'industry', 'economy']
        
        # Updated decision logic based on new criteria
        # Market Expectation: Call frequently for comprehensive trend analysis
        decision_market = any(keyword in query_lower for keyword in market_keywords)
        
        # Macro Analyst: Call for macro events affecting the sector
        decision_macro = any(keyword in query_lower for keyword in macro_keywords)
        
        # Revenue Segmentation: Call for fundamentals, business, products, revenue, cost, or legal impacts
        decision_revenue = any(keyword in query_lower for keyword in revenue_keywords)
        
        # Special case: If it's a stock-related query, always consider calling Market Expectation
        if any(word in query_lower for word in ['stock', 'ticker', 'company', 'firm', 'corporate']):
            decision_market = True
    ...
    async def _call_specific_agent(self, agent_name: str, query: str, ticker: str) -> Any:
        ...
        if agent_name == "Market_Expectation_Agent":
            ...
            # ✅ This agent is async and has progress tracking!
            result = await agent.process_query(query, ticker)
            agent.close()
            return result
    ...
    def print_analysis_results(self, user_query: str, ticker: str, result: ManagerAgentResult):
        ...
        # Add reasoning for why agents were called or skipped
        print("\n💭 Routing Reasoning:")
        query_lower = user_query.lower()
        
        # Market Expectation reasoning
        market_keywords = ['trend', 'pattern', 'historical', 'price', 'behavior', 'earnings', 'stock performance']
        market_matches = [kw for kw in market_keywords if kw in query_lower]
        if market_matches:
            print(f"   Market Expectation: ✅ Called because query contains: {', '.join(market_matches)}")
        else:
            print(f"   Market Expectation: ❌ Skipped - no market-related keywords found")
        
        # Revenue Segmentation reasoning
        revenue_keywords = ['revenue', 'partnership', 'business model', 'customer', 'service', 'profitability', 'cost', 'spending']
        macro_keywords = ['interest rate', 'federal reserve', 'economic', 'policy', 'macro', 'inflation', 'fed', 'powell']
        
        revenue_matches = [kw for kw in revenue_keywords if kw in query_lower]
        macro_matches = [kw for kw in macro_keywords if kw in query_lower]
        
        if revenue_matches:
            print(f"   Revenue Segmentation: ✅ Called because query contains revenue keywords: {', '.join(revenue_matches)}")
        elif macro_matches:
            print(f"   Revenue Segmentation: ✅ Called because query contains macro factors that affect business: {', '.join(macro_matches)}")
        else:
            print(f"   Revenue Segmentation: ❌ Skipped - no revenue or macro keywords found")
        
        # Macro Analyst reasoning
        if macro_matches:
            print(f"   Macro Analyst: ✅ Called because query contains macro factors: {', '.join(macro_matches)}")
        else:
            print(f"   Macro Analyst: ❌ Skipped - no macro-related keywords found")
...
async def analyze_with_multiprocessing(user_query: str, ticker: str, redis_config: Optional[Dict] = None, user_id: Optional[str] = None, manager_instance: Optional['ManagerAgent'] = None) -> Dict[str, Any]:
    ...
    result = process_manager_query(user_query, ticker)
    
    # Display routing decisions with detailed reasoning
    print(f"\n📊 ROUTING DECISIONS:")
    ...
    manager.print_analysis_results(user_query, ticker, result)
...
def process_manager_query(user_query: str, ticker: str) -> 'ManagerAgentResultPydantic':
    """
    Process a user query and intelligently route to appropriate agents
    This function matches the notebook version exactly
    """
    # Create LLM Call Agent instance
    ...
    # Get structured LLM
    structured_llm = manager_agent.get_structured_llm(ManagerAgentResultPydantic)
