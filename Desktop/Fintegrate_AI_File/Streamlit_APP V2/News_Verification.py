...
✅ Multiple credible sources (at least 3) confirm the EXACT EVENT exists
✅ Sources discuss the SAME company/stock/ticker mentioned
✅ Sources cover the SAME time period/quarter mentioned  
✅ Sources discuss the SAME financial metrics (earnings, revenue, etc.)
✅ Context analysis shows 70%+ match rate
...
❌ Contradictory information found between sources
❌ Insufficient source overlap (<70% context match)
❌ Mixed positive/negative signals for same event
...
================================================================
If the Verifier Agent failed, use the provided reason or error message to explain why the claim could not be verified. If the Evidence section is empty, says "No evidence available", or consists only of error / timeout messages →  return Json with only the following two keys:
...
• Analyze if multiple sources confirm the EVENT exists
• Check if sources discuss the same company/stock/period
• Determine if this is INVESTMENT-RELEVANT information
• ≤ 150-word concise reasoning focusing on event confirmation
...
async def async_verifier_agent(statement: str, sid: str):
    ...
    try:
        ...
        # Require at least 3 credible sources for verification
        if len(credit_results) < 3:
            err = {"content": f"Insufficient credible sources found for '{statement}'. Need at least 3, found {len(credit_results)}.", "passed": False}
...
def analyze_investment_context(statement: str, search_results: list) -> dict:
    ...
    for result in search_results:
        ...
        # If we have multiple context matches, this is likely the same event - MAKE MORE STRICT
        # Require BOTH company AND time period matches for better accuracy
        if (company_match or ticker_match) and time_match and metric_match:
            context_matches += 1
    ...
    is_same_event = context_percentage >= 70  # At least 70% of sources must discuss similar context
