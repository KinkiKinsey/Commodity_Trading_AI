...
def ml_moving_average(df, window=50, sigma=10, mult=2.0, forecast=0):
    ...
    current_trend = int(valid_df['trend'].iloc[-1])
    ...
    if len(trend_shifts) > 0:
        ...
        previous_trend = int(valid_df['trend'].iloc[last_shift_idx - 1]) if last_shift_idx > 0 else None
...
def graph(result):
    ...
    colors = ['#3179f5' if t == 1 else '#e91e63' for t in df['trend'][1:]]
    ...
    ax.fill_between(df['date'], df['lower'], df['upper'], color='blue', alpha=0.07)

    shift_points = np.where(np.diff(df['trend']) != 0)[0] + 1
    if len(shift_points) > 0:
        valid_shift_points = shift_points[shift_points < len(colors)]
        if len(valid_shift_points) > 0:
            ax.scatter(df.loc[valid_shift_points, 'date'], df['ML_MA'].iloc[valid_shift_points],
