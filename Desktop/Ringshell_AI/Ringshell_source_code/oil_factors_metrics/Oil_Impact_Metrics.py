...
# Add workspace root to path for LLM_Source, Data_Source imports
workspace_root = Path(__file__).parent.parent
if str(workspace_root) not in sys.path:
    sys.path.insert(0, str(workspace_root))

from LLM_Source.LLM_Call_Agent import LLMCallAgent
...
def step3_beta_filtering(
    ...
    from Data_Source.get_price import get_yahoo_data
...
if __name__ == "__main__":
    """
    ✅ CLEAN USAGE EXAMPLE - ONE FUNCTION, ONE JOB
    
    This module generates impact metrics from EXISTING LLM trend JSON.
    
    Usage:
        from Oil_Impact_Metrics import get_oil_impact_from_existing_trends
        from DataBase_Connection_Source.RedisDatabaseStorage import RedisDatabaseStorage
        
        # Get existing LLM trend data from Redis
        ...
        # Generate impact metrics (auto-calculates beta, calls LLM for factors)
        result = get_oil_impact_from_existing_trends(
        ...
        if result['status'] == 'success':
            print(f"Beta: {result['meta_info']['calculated_beta']:.4f}")
            datasets = result['datasets']
            print(datasets['risk_share_index'])
            print(datasets['factor_risk_reward_df'])
    """
    
    print("📋 Oil Impact Metrics Calculator")
    print("="*80)
    print("This module generates impact metrics from EXISTING LLM trend JSON.")
    print("")
    print("✅ ONE FUNCTION: get_oil_impact_from_existing_trends()")
    print("")
    print("What it does:")
    print("  1. Auto-calculates market beta")
    print("  2. Calls LLM to extract 6 MACRO + 6 MICRO factors")
    print("  3. Calls LLM to map factors to date ranges")
    print("  4. Calculates beta-filtered impacts (CAPM)")
    print("  5. Generates 7 impact datasets")
    print("")
    print("Input: llm_trend_json (from Redis - already have it!)")
    print("Output: 7 datasets + metadata")
    print("="*80)
