...
if __name__ == "__main__":
    # Example usage
    print("🚀 Factor Metrics Pipeline")
    print("="*80)
    
    # Run the pipeline
    result = get_factor_metrics_sync(
        ticker="CLZ25.NYM",
        language="Chinese"
    )
    ...
    if result['status'] == 'success':
        ...
        print(result['impact_metrics_df'].head())
        
        print("\n📅 Factor Time Ranges Preview:")
        print(result['factor_time_df'].head())
        
        # Optional: Save to CSV
        # result['impact_metrics_df'].to_csv('impact_metrics.csv')
        # result['factor_time_df'].to_csv('factor_time.csv')
    else:
        print(f"\n❌ Failed: {result['error']}")
