...
from Data_Source.wti_news import get_wti_news
...
async def deepseek_api_call(prompt: str, model: str = "deepseek-chat") -> str:
    """DeepSeek API call function - Same as Stock Trend Modular"""
    import asyncio
    from openai import OpenAI
    
    client = OpenAI(
        api_key="sk-43e9043c7ab8480393d34367f2ae997e",
        base_url="https://api.deepseek.com"
    )
    
    # Wrap synchronous call in asyncio.to_thread for TRUE concurrency
    response = await asyncio.to_thread(
        client.chat.completions.create,
        model=model,
        messages=[
            {"role": "system", "content": "You are a JSON-only financial analyst. Always respond with valid JSON only."},
            {"role": "user", "content": prompt + "\n\nCRITICAL: Respond ONLY with valid JSON, no text, no Chinese."}
        ],
        temperature=0.3,
        max_tokens=4000  # Same as Stock Trend Modular default
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        async def run_analysis():
            result = await get_llm_trend_summary(ticker, days)

            if "error" in result:
                print(f"❌ Error: {result['error']}")
            else:
                print(f"✅ Success! Generated LLM summary for {ticker}")
                
        asyncio.run(run_analysis())
    else:
        print("Usage: python LLM_Trend_Summary.py <ticker> [days]")