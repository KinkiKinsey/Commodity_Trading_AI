...
class RedisDatabaseStorage:
    ...
    def __init__(self):
        ...
        self.redis_client = redis.Redis(
            host='redis-14275.c83.us-east-1-2.ec2.redns.redis-cloud.com',
            port=14275,
            decode_responses=True,
            username="default",
            password="i84f74G6is5NgsjpqwM45OOY4Zd1ztE0"
        )
...
def redis_database_json(result: Any, database_name: str, *folder_names: str) -> Dict[str, str]:
    """Clean function to store JSON data in Redis"""
    storage = RedisDatabaseStorage()
    return storage.redis_database_json(result, database_name, *folder_names)

def redis_database_text(result: Any, database_name: str, *folder_names: str) -> Dict[str, str]:
    """Clean function to store text data in Redis"""
    storage = RedisDatabaseStorage()
    return storage.redis_database_text(result, database_name, *folder_names)

def redis_database_csv(result: Any, database_name: str, *folder_names: str) -> Dict[str, str]:
